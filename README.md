...
- **Auto-detect type** — searches all categories (artist, album, playlist, radio) and picks the best match
...
1. **Type** an artist, genre, mood, or playlist name into the search field
2. **Press Enter** or tap the ▶ button
3. The card searches ALL categories — **artist → playlist → album → radio** — and plays the best match with radio mode

Examples:
- `Daft Punk` → finds the artist, plays artist radio
- `lo-fi hip hop` → finds a playlist, plays playlist radio
- `chill jazz` → finds relevant artists, starts radio
- `BBC Radio 1` → finds the station, plays it

## How it works

The card uses a plain `HTMLElement` (no Lit, no framework dependencies) and communicates with HA via WebSocket:

1. Calls `music_assistant.search` with `return_response: true` across all media types
2. Picks the first result in priority order: artists → playlists → albums → radio
